...
constexpr int kProjectionFloats = 3 + 3 * 6;
...
struct alignas(16) SpectralSynthesisParamsNative {
    ...
    float padding;
    float meanRgb[4];
    float coefficientRgb[6][4];
};
static_assert(sizeof(SpectralSynthesisParamsNative) == 176);
...
void destroy_imported(SpectralSyncRenderer* renderer, ImportedBuffer& buffer) {
...
void destroy_host(SpectralSyncRenderer* renderer, HostBuffer& buffer) {
    if (buffer.buffer != VK_NULL_HANDLE) vkDestroyBuffer(renderer->device, buffer.buffer, nullptr);
    if (buffer.memory != VK_NULL_HANDLE) vkFreeMemory(renderer->device, buffer.memory, nullptr);
    buffer = {};
}

void cleanup_submission(SpectralSyncRenderer* renderer, PendingSubmission& submission) {
...
void reap_completed(SpectralSyncRenderer* renderer) {
...
bool create_uniform(
    SpectralSyncRenderer* renderer,
    HostBuffer& out,
    const SpectralSynthesisParamsNative& params
) {
    VkBufferCreateInfo bufferInfo{};
    bufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
...
bool create_acquire_semaphore(SpectralSyncRenderer* renderer, int& ownedFd, VkSemaphore& semaphore) {
...
bool create_release_semaphore(SpectralSyncRenderer* renderer, VkSemaphore& semaphore) {
...
void destroy_renderer(SpectralSyncRenderer* renderer) {
...
SpectralSyncRenderer* create_renderer(const std::uint32_t* spirv, std::size_t spirvBytes) {
    if (spirv == nullptr || spirvBytes == 0 || spirvBytes % 4 != 0) return nullptr;
    auto* renderer = new SpectralSyncRenderer();
    renderer->pending.reserve(kMaxPendingSubmissions);

    VkApplicationInfo appInfo{};
    appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    appInfo.pApplicationName = "LIFEOS MMSI Spectral SyncFD Forward";
...
jint dispatch_async(
    SpectralSyncRenderer* renderer,
    AHardwareBuffer* coefficientAhb,
    ...
    const VkDeviceSize sizes[4] = {
        pixels * 8 * sizeof(float),
        pixels * 4 * sizeof(float),
        pixels * sizeof(float),
        pixels * 4 * sizeof(float),
    };
    AHardwareBuffer* ahbs[4] = { coefficientAhb, normalDepthAhb, roughnessAhb, outputAhb };
    bool ok = true;
    ...
    if (ok) {
        ...
        infos[4].range = sizeof(SpectralSynthesisParamsNative);
...
bool unpack_projection(JNIEnv* env, jobject projectionBuffer, SpectralSynthesisParamsNative& params) {
    if (projectionBuffer == nullptr || env->GetDirectBufferAddress(projectionBuffer) == nullptr ||
        ...
        jclass exceptionClass = env->FindClass("java/lang/IllegalArgumentException");
        env->ThrowNew(exceptionClass, "Spectral RGB projection must contain 21 float values");
        return false;
    }
    const auto* projection = static_cast<const float*>(env->GetDirectBufferAddress(projectionBuffer));
    params.meanRgb[0] = projection[0];
    params.meanRgb[1] = projection[1];
    params.meanRgb[2] = projection[2];
    params.meanRgb[3] = 0.0f;
    int cursor = 3;
    for (int component = 0; component < 6; ++component) {
        params.coefficientRgb[component][0] = projection[cursor++];
        params.coefficientRgb[component][1] = projection[cursor++];
        params.coefficientRgb[component][2] = projection[cursor++];
        params.coefficientRgb[component][3] = 0.0f;
    }
    return true;
}
} // namespace
...
Java_app_lifeos_core_image_nativebackend_VulkanSpectralSyncFdHardwareBufferMmsiRenderer_nativeCreate(
    JNIEnv* env,
    jobject,
...
Java_app_lifeos_core_image_nativebackend_VulkanSpectralSyncFdHardwareBufferMmsiRenderer_nativeDispatch(
    JNIEnv* env,
    jobject,
    jlong handle,
    jobject coefficientBuffer,
    ...
    jfloat skyB,
    jfloat shadowFloor,
    jobject rgbProjectionBuffer,
    jint acquireFenceFd
) {
    AHardwareBuffer* coefficients = from_java(env, coefficientBuffer);
    ...
    if (handle == 0 || width <= 0 || height <= 0 || coefficients == nullptr || normalDepth == nullptr || roughness == nullptr || output == nullptr) {
        if (acquireFenceFd >= 0) close(acquireFenceFd);
        return kDispatchFailed;
    }

    SpectralSynthesisParamsNative params{};
    ...
    if (!unpack_projection(env, rgbProjectionBuffer, params)) {
        if (acquireFenceFd >= 0) close(acquireFenceFd);
        return kDispatchFailed;
    }

    return dispatch_async(
        reinterpret_cast<SpectralSyncRenderer*>(handle),
        coefficients,
        normalDepth,
        roughness,
        output,
...
Java_app_lifeos_core_image_nativebackend_VulkanSpectralSyncFdHardwareBufferMmsiRenderer_nativeDestroy(
    JNIEnv*,
    jobject,
    jlong handle
) {
    destroy_renderer(reinterpret_cast<SpectralSyncRenderer*>(handle));
}
