...
data class ModuleDomainKnowledgeState(
    ...
    init {
        ...
        require(bindings.all { it.domainId == domainId })
...
data class ModuleExpertiseState(
    ...
    init {
        ...
        require(learningEvidenceFingerprints.none { it.isBlank() })
        if (observedOutcomes == 0L) require(successfulOutcomeFraction == 0.0)
        if (learnedAdjustment != 0.0 || observedOutcomes > 0L) {
            require(learningEvidenceFingerprints.isNotEmpty())
        }
    }

    val effectiveExpertise: Double = exactDecimalSum(baselineExpertise, learnedAdjustment)
        .coerceIn(0.0, 1.0)
...
data class ModuleResourceProfile(
    ...
    init {
        ...
        require(maxParallelism in 1..256)
    }
    ...
    companion object {
        val LIGHT = ModuleResourceProfile(0.10, 0.10, 0.05, 0.0, 0.05, 4)
    }
}
