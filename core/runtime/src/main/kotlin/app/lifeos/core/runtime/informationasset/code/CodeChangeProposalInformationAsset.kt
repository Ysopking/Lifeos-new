...
data class CodeChangeStep(
    ...
    init {
        require(description.isNotBlank()) { "Code change description must not be blank" }
    }
}
...
/**
 * A proposal is not proof that a change was executed. This profile grounds only claims about the
 * current target and validation state in exact code-domain evidence; proposal intent/change text
 * remains proposal semantics and is never promoted to runtime activation authority here.
 */
                ...
                buildList {
                    if (codeEvidence.isEmpty()) {
                        add(requirement(
                            ...
                            message = "Code change target or validation claim requires code-domain evidence",
                        ))
                    } else if (codeEvidence.none { it.authority in ACCEPTED_AUTHORITIES }) {
                        add(requirement(
                            path = "claims[${claim.id.value}].authority",
                            message = "Code change target or validation claim cannot rely only on UNVERIFIED evidence",
                        ))
                    }
                }
            }
