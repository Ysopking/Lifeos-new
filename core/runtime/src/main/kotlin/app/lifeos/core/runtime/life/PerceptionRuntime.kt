...
// ---- B467 Owner-Authorized Observation Commit ----
...
data class OwnerAuthorizedObservationBatch(
    ...
    init {
        ...
        val allIds = authorized.map { it.id.value } + blocked.map { it.observationId.value }
...
/**
 * B467 applies Owner Observation Policy after adapter validation and before canonical Photon commit.
 *
 * Authorization only binds immutable provenance to the observed source event. The adapter cannot
 * mint grants, change the observed identity, promote an observation into fact/state, or gain any
