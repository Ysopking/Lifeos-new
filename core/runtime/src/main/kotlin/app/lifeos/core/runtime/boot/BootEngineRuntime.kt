...
data class BootEngineCycleLoadReport(
    ...
    init {
        ...
        require(health != BootEngineCycleStoreHealth.UNRECOVERABLE || message != null) {
            "Unrecoverable BootEngine cycle report requires a message"
        }
    ...
    val corrupted: Boolean
        get() = health == BootEngineCycleStoreHealth.UNRECOVERABLE
...
interface BootEngineCycleRepository {
    ...
    suspend fun loadReport(): BootEngineCycleLoadReport =
        BootEngineCycleLoadReport(
            ...
            health = BootEngineCycleStoreHealth.HEALTHY,
            message = null,
        )
}
...
fun interface BootEnginePerceptionBindingValidator {
    suspend fun matches(expected: PersonalContextBootBinding): Boolean
}
...
class BootEngineRuntime(
    ...
    private val perceptionBindingValidator: BootEnginePerceptionBindingValidator? = null,
) {
    ...
    suspend fun startCycle(
        ...
        if (frozenPerception != null && perceptionBindingValidator != null) {
            require(perceptionBindingValidator.matches(frozenPerception)) {
    ...
    suspend fun recover(): BootEngineRecoveryResult = mutex.withLock {
        ...
        require(cycleReport.health != BootEngineCycleStoreHealth.UNRECOVERABLE) {
            "BootEngine cycle store unavailable: ${cycleReport.message}"
        }
    ...
    private suspend fun perceptionBindingMatches(
        ...
        val validator = perceptionBindingValidator ?: return true
        return validator.matches(expected)
    }
