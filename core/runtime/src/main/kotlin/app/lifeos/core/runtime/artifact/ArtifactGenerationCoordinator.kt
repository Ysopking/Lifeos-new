            ...
            tags = buildSet {
                ...
                when (val profile = generation.profile) {
                    is DocumentArtifactProfile -> {
                        add("artifact-document-format:${profile.format}")
                    }
                    is CodeArtifactProfile -> {
                        add("artifact-code-language:${profile.language}")
                    }
    ...
    private fun StringBuilder.appendProfile(profile: ArtifactGenerationProfile) {
        ...
        when (profile) {
            is DocumentArtifactProfile -> {
                ...
                append(",\"style\":")
                profile.style?.let(::appendJson) ?: append("null")
            }
            is CodeArtifactProfile -> {
                append(",\"language\":"); appendJson(profile.language)
                append(",\"entrypoint\":")
                profile.entrypoint?.let(::appendJson) ?: append("null")
                append(",\"files\":[")
            ...
            is ImageArtifactProfile -> {
                ...
                append(",\"model\":")
                profile.model?.let(::appendJson) ?: append("null")
            }
        }
        append('}')
    }
