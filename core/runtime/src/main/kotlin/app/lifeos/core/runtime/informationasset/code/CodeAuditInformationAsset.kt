...
data class CodeAuditFinding(
    ...
    init {
        require(key.isNotBlank()) { "Code audit finding key must not be blank" }
        require(summary.isNotBlank()) { "Code audit finding summary must not be blank" }
        require(recommendation.isNotBlank()) { "Code audit recommendation must not be blank" }
    }
}
                ...
                buildList {
                    if (codeEvidence.isEmpty()) {
                        add(requirement(
                            ...
                            message = "Code audit finding requires code-domain evidence",
                        ))
                    } else if (codeEvidence.none { it.authority in ACCEPTED_FINDING_AUTHORITIES }) {
                        add(requirement(
                            path = "claims[${claim.id.value}].authority",
                            message = "Code audit finding cannot rely only on UNVERIFIED evidence",
                        ))
                    }
                }
            }
