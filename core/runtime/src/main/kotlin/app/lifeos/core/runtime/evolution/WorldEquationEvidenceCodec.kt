...
object WorldEquationEvidenceCodec {
    ...
    fun encode(record: WorldEquationEvidenceRecord): ByteArray =
        encodeVersion(record, VERSION)

    internal fun encodeLegacyV2ForTest(
        record: WorldEquationEvidenceRecord,
    ): ByteArray {
        require(!record.evidence.protocol.antiVacuityAttested) {
            "Legacy V2 encoding only supports unattested protocols"
        }
        require(
            ...
            "Legacy V2 encoding cannot represent exclusion evidence"
        }
        return encodeVersion(record, LEGACY_VERSION)
    }
    ...
    private fun writeEvidence(
        ...
        if (codecVersion == VERSION) {
            data.writeString(evidence.protocol.realizationProfileFingerprint)
            data.writeString(evidence.protocol.stateSpaceFingerprint)
            data.writeString(evidence.protocol.observableContractFingerprint)
            data.writeString(evidence.protocol.failureCriteriaFingerprint)
            data.writeInt(evidence.protocol.minimumExclusionRuns)
            data.writeDouble(evidence.protocol.maximumExclusionAbsoluteImprovement)
        }
    ...
    private fun readEvidence(
        ...
        val minimumActiveObservations = data.readInt()
        val minimumShadowRuns = data.readInt()
        val minimumHoldoutRuns = data.readInt()

        val protocol = if (codecVersion == LEGACY_VERSION) {
            ...
            WorldEquationEvaluationProtocol(
                ...
                maximumExclusionAbsoluteImprovement = data.readDouble(),
            )
        }
