...
class OutcomePhotonFactoryTest {
    ...
    fun `outcome photon is deterministic and preserves task prediction evidence provenance`() {
        ...
        assertEquals(setOf(sourcePhotonId), first.provenance.parentIds)
        ...
        assertEquals(1, first.relations.size)
        assertEquals(sourcePhotonId, first.relations.single().target)
        assertEquals(RelationType.DERIVED_FROM, first.relations.single().type)
    ...
    fun `failed task is recorded as finalized outcome without inventing verified success`() {
        ...
        assertTrue(photon.provenance.parentIds.isEmpty())
